...
const initialData = [
  {id:"BT001", code:"BZJTH-2026-008", bondPayId:"BD002", projectName:"南京化工园 PTA 装置升级", amount:128.6, returnFrom:"扬子石化", receiveDate:"2026-05-10", returnDate:"2026-05-15", status:"已退回"},
  {id:"BT002", code:"BZJTH-2026-007", bondPayId:"", projectName:"内蒙古某园区基础设施", amount:64, returnFrom:"鄂尔多斯城建", receiveDate:"2026-04-15", returnDate:"2026-04-20", status:"已退回"},
];

export function BondReturn() {
  ...
  const getStatusBadge = (status: string) => {
    if (status === '已退回') return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">已退回</Badge>;
    if (status === '退回中') return <Badge variant="outline" className="bg-amber-50 text-amber-700 border-amber-200">退回中</Badge>;
  ...
  const handleCreate = () => {
    setEditingItem({
      code: `BZJTH-${new Date().getFullYear()}-${String(data.length + 1).padStart(3, '0')}`,
      projectName: '',
      amount: 0,
      returnFrom: '',
  ...
  const handleSave = () => {
    if (editingItem.id) {
      ...
      setData([{ ...editingItem, id: `BT${Date.now()}` }, ...data]);
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">保证金退回管理</h2>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-slate-500 bg-slate-50/50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium">退回单位</th>
                  <th className="px-4 py-3 font-medium">接收日期</th>
                  <th className="px-4 py-3 font-medium">退回日期</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? filteredData.map((item, index) => (
                  <tr key={index} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">{item.returnFrom}</td>
                    <td className="px-4 py-3 text-slate-600">{item.receiveDate}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-900">{editingItem.id ? '编辑' : '新增'}保证金退回</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-indigo-600 uppercase tracking-widest flex items-center">退回单位</label>
                  <Input value={editingItem.returnFrom || ''} onChange={e => setEditingItem({...editingItem, returnFrom: e.target.value})} />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-semibold text-indigo-600 uppercase tracking-widest flex items-center">接收日期</label>
                  <Input type="date" value={editingItem.receiveDate || ''} onChange={e => setEditingItem({...editingItem, receiveDate: e.target.value})} />
                </div>
                
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-indigo-600 uppercase tracking-widest flex items-center">退回日期</label>
                  <Input type="date" value={editingItem.returnDate || ''} onChange={e => setEditingItem({...editingItem, returnDate: e.target.value})} />
                </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-950 border-l border-indigo-900/30 w-full max-w-2xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-slate-800 flex justify-between items-start">
              <div>
                <h3 className="text-2xl font-bold text-indigo-400 tracking-tight pr-8">保证金退回</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-lg p-5">
                ...
                <div className="grid grid-cols-[130px_1fr] gap-y-3 gap-x-4 text-sm">
                  ...
                  <div className="text-slate-500">退回单位</div><div className="text-slate-200">{viewingItem.returnFrom}</div>
                  <div className="text-slate-500">接收日期</div><div className="text-slate-200">{viewingItem.receiveDate}</div>
                  <div className="text-slate-500">退回日期</div><div className="text-slate-200">{viewingItem.returnDate}</div>
