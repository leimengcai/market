...
const initialData = [
  {id:"Q001", code:"ZG-2026-001", projectId:"M001", projectName:"云港新能源装置 EPC 项目", ownerName:"云港石化", reviewDate:"2026-04-08", reviewer:"陈鹏", result:"通过", notes:"资质满足"},
  {id:"Q002", code:"ZG-2026-002", projectId:"M004", projectName:"惠州炼化乙烯三期", ownerName:"中海石油", reviewDate:"2026-03-20", reviewer:"韩雨", result:"通过", notes:"业绩匹配"},
  {id:"Q003", code:"ZG-2026-003", projectId:"M005", projectName:"内蒙古某园区基础设施", ownerName:"鄂尔多斯城建", reviewDate:"2026-02-25", reviewer:"赵国伟", result:"通过", notes:""},
];
...
export function Prequalification() {
  ...
  const handleCreate = () => {
    setEditingItem({
      code: `ZG-${new Date().getFullYear()}-${String(data.length + 1).padStart(3, '0')}`,
      projectName: '',
      ownerName: '',
  ...
  const handleSave = () => {
    if (editingItem.id) {
      ...
      setData([{ ...editingItem, id: `Q${Date.now()}` }, ...data]);
    }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">资格预审</h2>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length > 0 ? filteredData.map((item, index) => (
                  <tr key={index} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-500 max-w-[200px] truncate" title={item.notes}>{item.notes || '—'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h3 className="text-lg font-bold text-slate-900">{editingItem.id ? '编辑' : '新增'}资格预审</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-indigo-600 uppercase tracking-widest flex items-center">业主</label>
                  <Input value={editingItem.ownerName || ''} onChange={e => setEditingItem({...editingItem, ownerName: e.target.value})} />
                </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-slate-950 border-l border-indigo-900/30 w-full max-w-2xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b border-slate-800 flex justify-between items-start">
              <div>
                <h3 className="text-2xl font-bold text-indigo-400 tracking-tight pr-8">资格预审台账</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-lg p-5">
                ...
                <div className="grid grid-cols-[130px_1fr] gap-y-3 gap-x-4 text-sm">
                  ...
                  <div className="text-slate-500">业主</div><div className="text-slate-200">{viewingItem.ownerName || '—'}</div>
